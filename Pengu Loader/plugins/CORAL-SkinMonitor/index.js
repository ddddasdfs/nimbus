...
const BRIDGE_PORT_STORAGE_KEY = "coral_bridge_port";
...
function publishSkinState(payload) {
  ...
  window.__coralSkinState = detail;
  try {
    window.__coralCurrentSkin = detail.name;
...
// window.__coralBridge is exposed in start() after port discovery completes,
// so that consumer plugins' waitForBridge() won't resolve until the port is known.
if (typeof window !== "undefined") {
  window.__coralBridgeEmit = sendBridgePayload; // backward compat (available early)
}
...
function setupBridgeSocket() {
  ...
  bridgeSocket.addEventListener("open", () => {
    ...
    window.__coralBridgeEmit = sendBridgePayload;
    _notifyReady();
  });
  ...
  bridgeSocket.addEventListener("message", (event) => {
    ...
    if (data && data.type === "skin-mods-response") {
      window.dispatchEvent(
        new CustomEvent("coral-custom-wheel-skin-mods", { detail: data })
      );
      return;
    }
    ...
    if (data && data.type === "maps-response") {
      window.dispatchEvent(
        new CustomEvent("coral-custom-wheel-maps", { detail: data })
      );
      return;
    }
    ...
    if (data && data.type === "fonts-response") {
      window.dispatchEvent(
        new CustomEvent("coral-custom-wheel-fonts", { detail: data })
      );
      return;
    }
    ...
    if (data && data.type === "announcers-response") {
      window.dispatchEvent(
        new CustomEvent("coral-custom-wheel-announcers", { detail: data })
      );
      return;
    }
    ...
    if (data && data.type === "category-mods-response") {
      window.dispatchEvent(
        new CustomEvent("coral-custom-wheel-category-mods", { detail: data })
      );
      return;
    }
    ...
    if (data && data.type === "others-response") {
      window.dispatchEvent(
        new CustomEvent("coral-custom-wheel-others", { detail: data })
      );
      return;
    }
    ...
    if (data && data.type === "champion-locked") {
      window.dispatchEvent(
        new CustomEvent("coral-custom-wheel-champion-locked", { detail: data })
      );
      return;
    }
    ...
    if (data && data.type === "phase-change" && data.phase === "Lobby") {
      ...
      window.dispatchEvent(new CustomEvent("coral-custom-wheel-reset"));
      return;
    }
...
async function start() {
  ...
  if (typeof window !== "undefined") {
    window.__coralBridge = Object.freeze({
