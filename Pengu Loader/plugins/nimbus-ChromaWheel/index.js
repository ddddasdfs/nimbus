...
(function createFakeChromaButton() {
  ...
  const SPECIAL_BASE_SKIN_IDS = new Set([99007]); // 82054, 145070, 103085, 25080 removed - handled by nimbus-FormsWheel
  const SPECIAL_CHROMA_SKIN_IDS = new Set([100001, 88888]); // 145071, 103086, 103087 removed - handled by nimbus-FormsWheel
  // HOL skins handled by nimbus-FormsWheel (should not show ChromaWheel buttons)
  ...
  // Shared bridge API (provided by nimbus-Bridge plugin)
  let bridge = null;
  ...
  function handleChromaStateUpdate(data) {
    ...
    // HOL chromas (Kai'Sa and Ahri) are now handled by nimbus-FormsWheel
    ...
    // Note: Mordekaiser handling removed - now handled by nimbus-FormsWheel plugin
    const getButtonIconPathForMordekaiser = (chromaId) => {
      // This function is kept for compatibility but should not be used
      // Mordekaiser is now handled by nimbus-FormsWheel
      return null;
    };

    // Helper to get buttonIconPath for Spirit Blossom Morgana forms
    // Note: Morgana handling removed - now handled by nimbus-FormsWheel plugin
    const getButtonIconPathForMorgana = (chromaId) => {
      // This function is kept for compatibility but should not be used
      // Morgana is now handled by nimbus-FormsWheel
      return null;
    };

    // Helper to get buttonIconPath for HOL chromas - removed, handled by nimbus-FormsWheel
    ...
    if (data.selectedChromaId && data.chromaColor) {
      ...
      if (
        ...
        // Note: Mordekaiser (82054) and Spirit Blossom Morgana (25080) handling removed - now handled by nimbus-FormsWheel plugin
        // HOL chromas (Kai'Sa and Ahri) are now handled by nimbus-FormsWheel - skip here
      } else {
      ...
      // Note: Mordekaiser (82054), Spirit Blossom Morgana (25080), and HOL chromas (Kai'Sa and Ahri) are now handled by nimbus-FormsWheel - skip here
  ...
  // HOL chroma functions removed - now handled by nimbus-FormsWheel
  ...
  // getHolButtonIconPath removed - now handled by nimbus-FormsWheel
  ...
  function ensureFakeButton(skinItem) {
    ...
    // Skip HOL skins - they are handled by nimbus-FormsWheel
    ...
    const hasChromas = Boolean(
      ...
      // Note: Mordekaiser (82054), Spirit Blossom Morgana (25080), and HOL skins removed - handled by nimbus-FormsWheel
    );
  ...
  function getChromaData(skinData) {
    ...
    // NOTE: Sahn Uzal Mordekaiser (82054) and Spirit Blossom Morgana (25080) removed - now handled by nimbus-FormsWheel plugin
    // Previously these special cases handled Mordekaiser and Morgana forms, but they're now excluded from this plugin

    // SPECIAL CASE: Risen Legend Kai'Sa and Ahri HoL skins are now handled by nimbus-FormsWheel
    // (removed - handled by nimbus-FormsWheel)
  ...
  function updateChromaPreview(chroma, chromaImage) {
    ...
    // Note: Mordekaiser removed - handled by nimbus-FormsWheel
  ...
  function updateChromaButtonColor() {
    ...
    buttons.forEach((button) => {
      ...
      // Note: Mordekaiser (82054), Spirit Blossom Morgana (25080), and HOL chromas are now handled by nimbus-FormsWheel
      const isElementalistLux =
        selectedChromaData &&
        (selectedChromaData.id === 99007 ||
          (selectedChromaData.id >= 99991 && selectedChromaData.id <= 99999));
      // HOL chromas are now handled by nimbus-FormsWheel
      const isDefault =
        !selectedChromaData ||
  ...
  function toggleChromaPanel(buttonElement, skinItem) {
    ...
    // Don't open panel for HOL skins - they are handled by nimbus-FormsWheel
    const currentSkinId = skinMonitorState?.skinId ?? null;
    if (currentSkinId && HOL_SKIN_IDS.has(currentSkinId)) {
  ...
  function subscribeToSkinMonitor() {
    ...
    if (window.__nimbusSkinState) {
      skinMonitorState = window.__nimbusSkinState;
