...
(function createCustomWheel() {
  const LOG_PREFIX = "[nimbus-CustomWheel]";
  ...
  const PANEL_ID = "nimbus-custom-wheel-panel-container";
  ...
  function setRightPaneMode(mode) {
    ...
    if (panel._rightTitle) {
      if (mode === "picker") {
        ...
        panel._rightTitle.innerHTML = `<span class="nimbus-wheel-title-icon">${icon}</span> Choose \u2022 ${escapeHtml(getTabLabel(activeTab))}`;
      } else {
  ...
  // Shared bridge API (provided by nimbus-SkinMonitor)
  let bridge = null;
  ...
  const CSS_RULES = `
    ...
    lol-uikit-flat-button.nimbus-custom-wheel-button,
    .nimbus-custom-wheel-button {
    ...
    .nimbus-custom-wheel-button .count-badge.social-count-badge,
    lol-uikit-flat-button.nimbus-custom-wheel-button .count-badge.social-count-badge,
    .nimbus-custom-wheel-button > .count-badge.social-count-badge,
    lol-uikit-flat-button.nimbus-custom-wheel-button > .count-badge.social-count-badge {
      position: absolute !important;
      /* Positioning: edit these via CSS variables on the element (DevTools-friendly) */
      top: var(--nimbus-badge-top, -4px) !important;
      right: var(--nimbus-badge-right, -17px) !important;
      left: var(--nimbus-badge-left, auto) !important;
      ...
      z-index: 1 !important;
      transform: translate(
        var(--nimbus-badge-translate-x, 60%),
        var(--nimbus-badge-translate-y, -60%)
      ) !important;
      margin: 0 !important;
    ...
    .${PANEL_CLASS} .nimbus-wheel-right-header {
      display: flex;
      align-items: center;
    ...
    .${PANEL_CLASS} .nimbus-wheel-right-title {
      font-weight: 700;
      color: #f0e6d2;
      font-size: 13px;
      overflow: hidden;
    ...
    .${PANEL_CLASS} .nimbus-wheel-right-title .nimbus-wheel-title-icon {
      width: 18px;
      height: 18px;
      flex-shrink: 0;
    }

    .${PANEL_CLASS} .nimbus-wheel-right-title .nimbus-wheel-title-icon svg {
      width: 18px;
      height: 18px;
      fill: none;
      stroke: #c8aa6e;
    ...
    .${PANEL_CLASS} .nimbus-wheel-summary {
      flex: 1;
      min-height: 0;
      display: flex;
      flex-direction: column;
    ...
    .${PANEL_CLASS} .nimbus-wheel-summary::-webkit-scrollbar { width: 6px; }
    .${PANEL_CLASS} .nimbus-wheel-summary::-webkit-scrollbar-track { background: rgba(0,0,0,0.3); }
    .${PANEL_CLASS} .nimbus-wheel-summary::-webkit-scrollbar-thumb { background: #5b5a56; border-radius: 3px; }

    .${PANEL_CLASS} .nimbus-wheel-summary-row {
      display: grid;
    ...
    .${PANEL_CLASS} .nimbus-wheel-summary-row.active {
      border-left: 3px solid #c8aa6e;
    }

    .${PANEL_CLASS} .nimbus-wheel-summary-row:hover .nimbus-wheel-summary-icon {
      color: #c8aa6e;
    }

    .${PANEL_CLASS} .nimbus-wheel-summary-icon {
      width: 18px;
      height: 18px;
      flex-shrink: 0;
      color: #5b5a56;
    ...
    .${PANEL_CLASS} .nimbus-wheel-summary-icon svg {
      width: 18px;
      height: 18px;
      fill: none;
      stroke: currentColor;
    ...
    .${PANEL_CLASS} .nimbus-wheel-summary-left {
      flex: 1;
      min-width: 0;
      display: flex;
      flex-direction: column;
    ...
    .${PANEL_CLASS} .nimbus-wheel-summary-label {
      color: #a09b8c;
      font-size: 12px;
      font-weight: 700;
    ...
    .${PANEL_CLASS} .nimbus-wheel-summary-value {
      color: #f0e6d2;
      font-size: 13px;
      font-weight: 700;
    ...
    .${PANEL_CLASS} .nimbus-wheel-picker {
      flex: 1;
      min-height: 0;
      display: none;
    }

    .${PANEL_CLASS} .nimbus-wheel-picker.active {
      display: flex;
      flex-direction: column;
      min-height: 0;
    }
  ...
  function injectCSS() {
    const styleId = "nimbus-custom-wheel-css";
  ...
  function createButton() {
    ...
    button.className = "lol-uikit-flat-button idle nimbus-custom-wheel-button";
    ...
    countBadge.style.setProperty("--nimbus-badge-top", "-4px");
    countBadge.style.setProperty("--nimbus-badge-right", "-17px");
    countBadge.style.setProperty("--nimbus-badge-left", "auto");
    countBadge.style.setProperty("--nimbus-badge-translate-x", "60%");
    countBadge.style.setProperty("--nimbus-badge-translate-y", "-60%");
  ...
  function createPanel() {
    ...
    rightHeader.className = "nimbus-wheel-right-header";

    const rightTitle = document.createElement("div");
    rightTitle.className = "nimbus-wheel-right-title";
    ...
    summaryView.className = "nimbus-wheel-summary";
    ...
    SUMMARY_TABS.forEach((tab) => {
      ...
      row.className = "nimbus-wheel-summary-row";

      // Left cell: value
      const left = document.createElement("div");
      left.className = "nimbus-wheel-summary-left";

      const label = document.createElement("div");
      label.className = "nimbus-wheel-summary-label";
      label.style.display = "flex";
      label.style.alignItems = "center";
      label.style.gap = "6px";

      const iconSpan = document.createElement("span");
      iconSpan.className = "nimbus-wheel-summary-icon";
      ...
      value.className = "nimbus-wheel-summary-value";
    ...
    pickerView.className = "nimbus-wheel-picker";
  ...
  function handleModSelect(modId, buttonElement, modData) {
    ...
    if (selectedModId === modId) {
      ...
      const state = window.__nimbusSkinState || {};
      ...
      if (championId && emitSkinId) {
        ...
        console.log(`[nimbus-CustomWheel] Sending mod selection:`, payload);
        if (bridge) bridge.send(payload);
      } else {
        console.warn(`[nimbus-CustomWheel] Cannot send mod selection - missing championId or skinId:`, { championId, skinId: emitSkinId });
      }
    }
  ...
  // Backend should look in: %LOCALAPPDATA%\nimbus\mods\maps
  ...
  function handleModsResponse(event) {
    ...
    const liveSkinId = Number((window.__nimbusSkinState || {}).skinId) || skinId;
