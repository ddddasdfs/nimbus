# nimbus — Bundled Binary Provenance & Verification

nimbus's core Python and JavaScript are auditable source. This document tracks the
...
- **The Rose build ADDS custom headless CLI flags** — `--force-activate`,
  `--force-deactivate`, `--set-league-path`, `--restart-client`, `--silent` — that nimbus's
  ...
  drop-in swap to the official binary would break nimbus's auto-activation; using official
...
- **`Pengu Loader.exe` + `core.dll`:** Custom "Rose Loader" fork — cannot be verified
  ...
  (code-signed; test that the nimbus-* plugins still load) or obtain and audit the fork's
  source and rebuild.
...
2. **Replace `Pengu Loader.exe`/`core.dll`** with official Pengu Loader v1.1.6, or vendor
   and audit the Rose-Pengu fork source and rebuild. Confirm the `nimbus-*` plugins load
   after any swap.
