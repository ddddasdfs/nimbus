...
# -*- coding: utf-8 -*-
"""
Complete build script for 2SDAY
...
def run_create_installer():
    ...
    installer_files = list(installer_dir.glob("2SDAY_Setup*.exe"))
...
def build_all():
    ...
    print_header("2SDAY - Complete Build Process")
    ...
    if not run_build_exe():
        ...
        print("   pip install -r requirements.txt")
        print("2. Close any running instances of 2SDAY.exe")
    ...
    exe_path = Path("dist/2SDAY/2SDAY.exe")
    installer_files = list(Path("installer").glob("2SDAY_Setup*.exe"))
    ...
    print("    Run: dist\\2SDAY\\2SDAY.exe")
    print()
    print("  • For distribution:")
    print(f"    Share: {installer_path if installer_path else 'installer/2SDAY_Setup.exe'}")
    print()
    print("  • For portable version:")
    print("    Zip: dist\\2SDAY\\ folder")
...
def main():
    ...
    if not Path("main.py").exists():
        ...
        print("Please run this script from the 2SDAY root directory.")
        sys.exit(1)
