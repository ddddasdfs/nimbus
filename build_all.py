...
# -*- coding: utf-8 -*-
"""
Complete build script for Coral
...
if sys.version_info < MIN_PYTHON:
    sys.stderr.write(
        f"Coral build scripts require Python {MIN_PYTHON[0]}.{MIN_PYTHON[1]} or newer.\n"
...
def run_build_exe():
    ...
    exe_path = Path("dist/Coral/Coral.exe")
...
def run_create_installer():
    ...
    installer_files = list(installer_dir.glob("Coral_Setup*.exe"))
...
def build_all():
    ...
    print_header("Coral - Complete Build Process")
    ...
    if not run_build_exe():
        ...
        print("   pip install -r requirements.txt")
        print("2. Close any running instances of Coral.exe")
    ...
    exe_path = Path("dist/Coral/Coral.exe")
    installer_files = list(Path("installer").glob("Coral_Setup*.exe"))
    ...
    print("    Run: dist\\Coral\\Coral.exe")
    print()
    print("  • For distribution:")
    print(f"    Share: {installer_path if installer_path else 'installer/Coral_Setup.exe'}")
    print()
    print("  • For portable version:")
    print("    Zip: dist\\Coral\\ folder")
...
def main():
    ...
    if not Path("main.py").exists():
        ...
        print("Please run this script from the Coral root directory.")
        sys.exit(1)
