...
# -*- coding: utf-8 -*-
"""
Complete build script for nimbus
...
def run_create_installer():
    ...
    installer_files = list(installer_dir.glob("nimbus_Setup*.exe"))
...
def build_all():
    ...
    print_header("nimbus - Complete Build Process")
    ...
    if not run_build_exe():
        ...
        print("   pip install -r requirements.txt")
        print("2. Close any running instances of nimbus.exe")
    ...
    exe_path = Path("dist/nimbus/nimbus.exe")
    installer_files = list(Path("installer").glob("nimbus_Setup*.exe"))
    ...
    print(f"    Share: {installer_path if installer_path else 'installer/nimbus_Setup.exe'}")
    print()
...
def main():
    ...
    if not Path("main.py").exists():
        ...
        print("Please run this script from the nimbus root directory.")
        sys.exit(1)
