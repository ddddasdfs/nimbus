...
APP_VERSION = "1.4.0"                           # Application version
APP_USER_AGENT = f"nimbus/{APP_VERSION}"  # User-Agent header for HTTP requests
...
# Lock file name
LOCK_FILE_NAME = "nimbus.lock"

# NEW: Windows named mutex for single-instance (per-user/session)
_IS_DEV_BUILD = bool(getattr(sys, "frozen", False)) and (
    "nimbusdev" in Path(sys.executable).stem.lower() or "nimbus-dev" in Path(sys.executable).stem.lower()
)
SINGLE_INSTANCE_MUTEX_NAME = r"Local\NimbusDevSingleInstance" if _IS_DEV_BUILD else r"Local\NimbusSingleInstance"

# Log file patterns (handles .log files)
LOG_FILE_PATTERN = "nimbus_*.log*"
...
# =============================================================================
# ANALYTICS CONSTANTS  (removed in nimbus)
# =============================================================================
#
# Upstream nimbus sent a periodic "ping" containing the machine's persistent
# Windows Machine GUID to an external analytics server every 5 minutes, on by
# default and without consent. nimbus removes analytics entirely: the analytics/
