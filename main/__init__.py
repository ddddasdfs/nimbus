...
if sys.version_info < MIN_PYTHON:
    raise RuntimeError(
        f"Coral requires Python {MIN_PYTHON[0]}.{MIN_PYTHON[1]} or newer. "
...
def _check_dll_present() -> bool:
    ...
    content_text = (
        "Due to DMCA restrictions, Coral cannot distribute the cslol-dll.dll file.\n\n"
        "You must provide your own signed cslol-dll.dll file.\n\n"
        "See the project README for how to obtain and sign this file yourself.\n\n"
        "<a href=\"https://github.com/ddddasdfs/Coral\">https://github.com/ddddasdfs/Coral</a>"
    )
    ...
    config.pszWindowTitle = "Coral - DLL Required"
...
def _update_registry_version() -> None:
    ...
    try:
        import winreg
        key_path = r"SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Coral"
...
def run_league_unlock(args: Optional[argparse.Namespace] = None,
    ...
    """Run the core Coral application startup and main loop."""
...
def main() -> None:
    """Program entry point that prepares and launches Coral."""
    ...
    try:
        ...
        try:
            ...
            report_issue(
                "FATAL_CRASH",
                "error",
                "Coral crashed unexpectedly.",
                details={"type": type(e).__name__, "error": str(e)},
                hint="Check %LOCALAPPDATA%\\Coral\\logs\\ for details.",
            )
        except Exception:
            pass
...
Log location: Check %LOCALAPPDATA%\\Coral\\logs\\
================================================================================
"""
        ...
        if sys.platform == "win32":
            try:
                ctypes.windll.user32.MessageBoxW(
                    0,
                    f"Coral crashed with an unhandled error:\n\n{str(e)}\n\nError type: {type(e).__name__}\n\nPlease check the log file in:\n%LOCALAPPDATA%\\Coral\\logs\\",
                    "Coral - Fatal Error",
