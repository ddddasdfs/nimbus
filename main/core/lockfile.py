...
def create_lock_file() -> bool:
    ...
    try:
        ...
        lock_file_path = state_dir / "coral.lock"
        app_state = get_app_state()
        ...
        if not create_single_instance_mutex():
            try:
                ctypes.windll.user32.MessageBoxW(
                    0,
                    "Another instance of Coral is already running!\n\nPlease close the existing instance before starting a new one.",
                    "Coral - Instance Already Running",
                    0x50010,
                )
            except Exception:
                log.error("Another instance of Coral is already running!")
            sys.exit(1)
        return
