...
def setup_logging(log_mode: str = 'customer', *, write_logs: bool = True):
    ...
    if write_logs:
        try:
            ...
            log_file = logs_dir / f"nimbus_{timestamp}.log"
    ...
    if sys.stdout is not None and not (hasattr(sys.stdout, 'name') and sys.stdout.name == os.devnull):
        try:
            ...
            if _CURRENT_LOG_MODE == 'customer':
                ...
                if log_file:
                    logger.info(f"nimbus Started (Log: {log_file.name})")
                else:
                    logger.info("nimbus Started (logs disabled)")
            else:
                # Detailed startup for verbose/debug modes
                logger.info("=" * LOG_SEPARATOR_WIDTH)
                if log_file:
                    logger.info(f"nimbus - Starting... (Log file: {log_file.name})")
                else:
                    logger.info("nimbus - Starting... (logs disabled)")
                logger.info("=" * LOG_SEPARATOR_WIDTH)
