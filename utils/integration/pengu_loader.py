...
around the executable bundled alongside Coral.
"""
...
def _sanitize_plugin_entrypoints(pengu_dir: Path) -> None:
    ...
    - In frozen builds, Coral overlays the bundled `Pengu Loader` onto the runtime directory.
    ...
    try:
        ...
        # Non-fatal: never block Coral launch for a best-effort cleanup.
...
def _resolve_pengu_dir() -> Path:
    ...
    try:
        ...
        #   %LOCALAPPDATA%\Coral\Pengu Loader\plugins
...
def activate_on_start(league_path: Optional[str] = None) -> bool:
    """
    Force activate Pengu Loader when Coral launches.
...
def deactivate_on_exit() -> bool:
    """
    Force deactivate Pengu Loader when Coral shuts down.
